...
extern qvector <VTBL_info_t> vtbl_t_list; // virtual table type list
...
struct obj_fint_t : public ctree_parentee_t
{
	...
	int idaapi visit_expr(cexpr_t *e); // overwrite
	
	obj_fint_t() : bFound(false) {} // ctor
};
...
void get_struct_key(struc_t * struc_type, const  VTBL_info_t &vtbl_info, qstring &file_entry_key, bool &filtered, std::map<ea_t, VTBL_info_t> vtbl_map) {
	qstring sub_key;
