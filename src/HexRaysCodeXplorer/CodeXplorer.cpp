...
#define ROOT "*&|"
		...
		int idaapi visit_expr(cexpr_t *e)
		{
			...
			if (e->op == cot_asg && e->x->op == cot_var && e->y->op == cot_var)
			{
				...
				for (unsigned int i = 0; i < roots[lvar_name].size(); i++)
				{
				...
				sprintf_s(pstx_buf, "%d", postfixes[rvar_name]++);
...
static bool idaapi show_current_citem_to_custom_view(void *ud)
{
	vdui_t &vu = *(vdui_t *)ud;
	vu.get_current_item(USE_KEYBOARD);
...
char szfunc_addr[] = "//---------(0x%0x)---------------------------\n";

static bool idaapi export_pseudocode_to_file(void *ud)
{
	vdui_t &vu = *(vdui_t*)ud;
	...
	char func_name[MAXSTR] = { 0x0 };
	
	if (get_func_name(pfn->startEA, func_name, MAXSTR) != NULL)
	{
		char * file_to_save = qstrncat(func_name, ".cpp", MAXSTR);

		char *fname = askfile_c(1, file_to_save, "Pseudocode Save ?");
		...
		if (fp != NULL) {
			ewrite(fp, headers, sizeof(headers) - 1);
			...
			for (unsigned int i = 0; i < sv.size(); i++)
			{
				...
				ewrite(fp, buf, (ssize_t)strlen(buf));
				ewrite(fp, "\n", 1);
			}
			msg("Saved to %s...\n", fname);
			eclose(fp);
...
static int idaapi callback(void *, hexrays_event_t event, va_list va)
{
	...
	case hxe_right_click:
	{
		...
		add_custom_viewer_popup_item(vu.ct, "Ctree Item View", hotkey_vc, show_current_citem_to_custom_view, &vu);
		...
		add_custom_viewer_popup_item(vu.ct, "Save Pseudocode ...", NULL, export_pseudocode_to_file, &vu);
	}
	break;
	...
	case hxe_keyboard:
	{
		...
		if (keycode == hotcode_vc)
			return show_current_citem_to_custom_view(&vu);
