...
static bool check_vtable_load_instruction(ea_t ea_code) {
	...
	else if ((*((PUINT)disasm_line) == 0x2061656c /*"lea "*/) && (strstr(disasm_line + 4, "rcx") != NULL) && ((strstr(disasm_line + 4, "off_") != NULL) || (strstr(disasm_line + 4, "const") != NULL))) {
		is_move_xref = true;
	}
...
void find_vtables_rtti()
{
	...
	for (std::map<ea_t, vftable::vtinfo>::iterator it = rtti_vftables.begin(); it != rtti_vftables.end(); ++it) {
		VTBL_info_t vftable_info_t;
		vftable::vtinfo second = it->second;
		vftable_info_t.ea_begin = second.start;
		vftable_info_t.ea_end = second.end;
		vftable_info_t.methods = second.methodCount;
		vftable_info_t.vtbl_name = second.type_info;
...
#else
		tmp.cat_sprnt(_T(" 0x%016llx:  %s"), addr, name);
#endif // !#ifndef __EA64__
...
static bool idaapi ct_vtbl_xrefs_window_dbclick(TCustomControl *v, int shift, void *ud)
{
	int x, y;
	place_t *place = get_custom_viewer_place(v, true, &x, &y);
	// When people click outside the list
...
static bool idaapi show_vtbl_xrefs_window_cb(void *ud)
{
	...
	if (!xref_list.empty())
	{
		...
		set_custom_viewer_handlers(si->cv, NULL, NULL, ct_vtbl_xrefs_window_dbclick, NULL, NULL, si);
#else
		set_custom_viewer_handlers(si->cv, NULL, NULL, NULL, ct_vtbl_xrefs_window_dbclick, NULL, NULL, si);
#endif
...
// Display RTTI Function List from N-SEC 1.7Ed
